import { useRef } from 'react';
import { useNavigate, useParams, useLocation } from 'react-router-dom';
import { useLanguage } from '../i18n/LanguageContext';
import { usePaymentMethods } from '../hooks/usePaymentMethods';
import PaymentBrandMark from '../components/wallet/PaymentBrandMark';
import TransactionSuccessShell from '../components/ui/TransactionSuccessShell';
import AddressMapThumb from '../components/business/AddressMapThumb';
import { Marker } from 'react-map-gl/maplibre';

export interface BusinessPaymentSuccessState {
  amount: number;
  cashback?: number;
  businessName?: string;
  businessNameHe?: string;
  businessLogo?: string;
  /** Could be a voucher id, card id, etc. */
  paymentMethodId?: string;
  returnTo?: string;
  /** Business location for the map pin */
  businessLng?: number;
  businessLat?: number;
}

export default function BusinessPaymentSuccessPage() {
  const { lang = 'he' } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const { language } = useLanguage();
  const isHe = language === 'he';

  const state = (location.state as BusinessPaymentSuccessState | null) ?? { amount: 150 };
  const {
    amount = 150,
    cashback = Math.round(amount * 0.05 * 100) / 100,
    businessName,
    businessNameHe,
    businessLogo,
    paymentMethodId,
    returnTo,
    businessLng = 34.7818,
    businessLat = 32.0853,
  } = state;

  const { data: paymentMethods } = usePaymentMethods();
  const payMethod = paymentMethods.find((m) => m.id === paymentMethodId) ?? paymentMethods[0];

  const confirmId = useRef(`NX-${new Date().getFullYear()}-${Math.floor(10000 + Math.random() * 90000)}`);
  const now = useRef(new Date());

  const timeStr = now.current.toLocaleTimeString(isHe ? 'he-IL' : 'en-IL', { hour: '2-digit', minute: '2-digit' });
  const dateStr = now.current.toLocaleDateString(isHe ? 'he-IL' : 'en-IL', { day: '2-digit', month: '2-digit', year: '2-digit' });

  const displayBusiness = isHe ? (businessNameHe ?? businessName) : businessName;

  const handleClose = () => navigate(returnTo ?? `/${lang}/wallet`, { replace: true });

  return (
    <TransactionSuccessShell
      isHe={isHe}
      cashback={cashback}
      onClose={handleClose}
    >
      {/* Header */}
      <div className="px-5 pt-5 pb-4 text-center">
        {businessLogo && (
          <div className="flex justify-center mb-3">
            <img src={businessLogo} alt="" draggable={false} className="w-12 h-12 object-contain rounded-xl border border-border/60" />
          </div>
        )}
        {displayBusiness && (
          <p className="text-sm text-text-secondary font-medium mb-1">{displayBusiness}</p>
        )}
        <p className="text-[44px] font-bold text-text-primary leading-tight" dir="ltr">
          ₪{amount}
        </p>
        <span className="inline-block mt-2 text-[12px] font-semibold text-green-700 bg-green-50 px-3 py-1 rounded-full">
          {isHe ? 'עסקה בוצעה' : 'Payment successful'}
        </span>
      </div>

      <div className="mx-5 border-t border-border/60" />

      {/* Detail rows */}
      <div className="px-5 py-4 space-y-3" dir={isHe ? 'rtl' : 'ltr'}>
        {[
          { label: isHe ? 'סטטוס'     : 'Status', value: isHe ? 'הושלם' : 'Completed', green: true  },
          { label: isHe ? 'תאריך'      : 'Date',   value: dateStr,                      green: false },
          { label: isHe ? 'שעה'        : 'Time',   value: timeStr,                      green: false },
          { label: isHe ? 'מספר אישור' : 'Ref',    value: confirmId.current,            green: false },
        ].map(({ label, value, green }) => (
          <div key={label} className="flex items-center justify-between text-[14px]">
            <span className="text-text-secondary">{label}</span>
            <span className={`font-medium ${green ? 'text-green-600' : 'text-text-primary'}`} dir="ltr">
              {green && <span className="me-1">✓</span>}
              {value}
            </span>
          </div>
        ))}
      </div>

      {/* Payment method */}
      {payMethod && (
        <>
          <div className="mx-5 border-t border-border/60" />
          <div className="mx-5 my-4 bg-surface rounded-2xl p-4">
            <p className="text-[12px] font-semibold text-text-secondary mb-3 uppercase tracking-wide">
              {isHe ? 'אמצעי תשלום' : 'Payment method'}
            </p>
            <div className="flex items-center gap-3">
              <PaymentBrandMark brand={payMethod.brand} />
              <span className="text-text-secondary font-medium text-sm tracking-widest" dir="ltr">
                {payMethod.last4 ? `• • • •  ${payMethod.last4}` : (isHe ? payMethod.labelHe : payMethod.label)}
              </span>
            </div>
          </div>
        </>
      )}

      {/* Business location map */}
      <div className="mx-5 mb-4">
        <div className="mx-5 border-t border-border/60 mb-4 -mx-0" />
        <p className="text-[12px] font-semibold text-text-secondary mb-2 uppercase tracking-wide">
          {isHe ? 'מיקום בית העסק' : 'Business location'}
        </p>
        <div className="w-full h-44 rounded-2xl overflow-hidden border border-border/60">
          <AddressMapThumb
            lng={businessLng}
            lat={businessLat}
            zoom={15}
            interactive={false}
          >
            <Marker longitude={businessLng} latitude={businessLat} anchor="bottom">
              <div className="flex flex-col items-center gap-1" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}>
                {businessLogo ? (
                  <div className="w-9 h-9 rounded-xl overflow-hidden bg-white border-2 border-white shadow-md">
                    <img src={businessLogo} alt="" draggable={false} className="w-full h-full object-contain" />
                  </div>
                ) : (
                  <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center shadow-md">
                    <span className="material-symbols-outlined text-white" style={{ fontSize: 18 }}>store</span>
                  </div>
                )}
                {displayBusiness && (
                  <span className="text-[10px] font-bold text-white bg-black/60 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                    {displayBusiness}
                  </span>
                )}
              </div>
            </Marker>
          </AddressMapThumb>
        </div>
      </div>
    </TransactionSuccessShell>
  );
}
