...
export default function AddMoneyLoadingPage() {
  ...
  const state = location.state as { amount?: number; sourceId?: string } | null;
  const amount = state?.amount ?? 150;
  const sourceId = state?.sourceId;

  const { data: paymentMethods } = usePaymentMethods();
  const payMethod = paymentMethods.find((m) => m.id === sourceId) ?? paymentMethods[0];
  ...
  const handleClose = () => navigate(`/${lang}/wallet`, { replace: true, state: { addedAmount: amount } });
    ...
    >
      ...
      <div className="px-5 pt-5 pb-4 text-center">
        <p className="text-sm text-text-secondary font-medium">
          {isHe ? 'הכסף הגיע לארנק' : 'Money added to wallet'}
        </p>
        <p className="text-[44px] font-bold text-text-primary leading-tight mt-1" dir="ltr">
          ₪{amount}
        </p>
