...
interface GiftCardsCarouselProps {
  /** Height of the carousel stage in px (default 320). */
  height?: number;
  /** Show the floating "X redeemed!" toasts (default true). */
  showNotifications?: boolean;
}

/**
 * The rotating 3D gift-card carousel from the onboarding "gift cards" story,
 * extracted so it can be reused (e.g. as the hero of the deal-intro page).
 * Title and page chrome live with the caller — this renders only the animation.
 */
export default function GiftCardsCarousel({ height = 320, showNotifications = true }: GiftCardsCarouselProps) {
  ...
  useEffect(() => {
    if (isPulsing && showNotifications) {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto" style={{ height }}>
      ...
      {/* Notifications */}
      {showNotifications && notifications.map((n, i) => (
        <motion.div
          key={n.id}
