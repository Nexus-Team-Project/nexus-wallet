...
export default function LoginSheet() {
  ...
  const appleButton = (variant: 'dark' | 'light') => (
    <button
      onClick={handleApple}
      disabled={isLoading}
      className={`relative w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-bold active:scale-[0.98] transition-all disabled:opacity-50 ${
        variant === 'dark'
          ? 'bg-bg-dark text-white'
          : 'bg-white border border-border text-text-primary hover:bg-surface'
      }`}
  ...
  const whatsappButton = (
    <button
      ...
      disabled={isLoading}
      className="relative w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl bg-white border border-border text-sm font-bold text-text-primary hover:bg-surface active:scale-[0.98] transition-all disabled:opacity-50"
    >
