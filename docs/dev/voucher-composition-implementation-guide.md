...
## 2. Files — what to copy and what to adapt
...
| `src/pages/VoucherPurchasePage.tsx` | Port by section | All integration points listed in §5. House style keeps sub-components inline in this file. |
| `src/pages/VoucherSuccessPage.tsx` | Add 2 fields + 1 row | §5.8 |

## 3. The pure util — contract you must preserve
...
## 4. State and derivations — exact wiring
...
## 5. UI integration points (top of page → bottom)

### 5.1 Card gallery (dual-mode deck)
...
### 5.2 Composition banner — below the gallery, ABOVE the amount input
...
## 6. Business-rule interactions (intentional — do not "fix")
...
## 7. Pitfalls we already hit — read before porting
...
## 9. Reference commits (branch `raz`)
